...
import { capitalize, queryStringForArray } from 'helpers/stringHelpers';
...
class Body extends Component {
      ...
      >
        ...
        <div>
          ...
          >
            ...
            <SideMenu
              menuItems={['all', 'post', 'comment', 'video', 'url']}
              url={route.url}
              onClickFeedMenu={this.onClickFeedMenu}
              style={{ width: '30rem' }}
            />
          </div>
          {loaded &&
  ...
  onClickFeedMenu = ({ type, url }) => {
    console.log(type, url);
    const { clearFeeds, disableAutoscroll, history } = this.props;
    clearFeeds();
    disableAutoscroll();
    history.push(
      `${url}${type !== 'all' ? `/${type === 'url' ? 'link' : type}s` : ''}`
    );
  };
