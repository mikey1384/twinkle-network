...
export default function TweenState(InnerComponent) {
  let rafID = null;
  class TweenStateComponent extends Component {
    constructor() {
      super()
      this.state = {
        tweenQueue: []
    ...
    tweenState(path, {easing, duration, delay, beginValue, endValue, onEnd, stackBehavior: configSB}) {
      this.setState(state => {
        ...
        if (newTweenQueue.length === 1) {
          rafID = requestAnimationFrame(this.props.rafCb.bind(this));
        }
