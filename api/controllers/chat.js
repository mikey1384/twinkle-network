...
router.get('/', requireAuth, (req, res) => {
  ...
  return fetchChat({
    user,
    channelId: Number(channelId) || lastChannelId || generalChatId
  })
  ...
  return poolQuery(postSubjectQuery, {
    ...
    .then(({ insertId: subjectId }) =>
      poolQuery(postMessageQuery, {
        ...
        isSubject: true,
        subjectId
      }).then(({ insertId }) => Promise.resolve({ subjectId, insertId }))
    )
    .then(({ subjectId, insertId }) => {
      return poolQuery(
        `UPDATE msg_channels SET currentSubjectId = ? WHERE id = 2`,
        subjectId
      ).then(() =>
        res.send({
          subjectId,
...
router.get('/chatSubject/messages', (req, res) => {
  ...
  `
  return poolQuery(query, subjectId)
    .then(messages => {
...
router.put('/chatSubject/reload', requireAuth, (req, res) => {
  ...
  return poolQuery(subjectQuery, subjectId)
    .then(([subject]) => {
      const message = {
        channelId: 2,
        userId: user.id,
      ...
      return promiseSeries([
        () =>
          poolQuery(`UPDATE msg_channels SET ? WHERE id = 2`, {
            currentSubjectId: subjectId
          }),
        () =>
          poolQuery(`UPDATE content_chat_subjects SET ? WHERE id = ?`, [
            post,
            subjectId
          ]),
        () =>
      ...
      ]).then(([, , [{ numMsgs }], { insertId: messageId }]) => {
...
router.put('/message', requireAuth, async(req, res) => {
  ...
  const query = `
    UPDATE msg_chats SET ? WHERE id = ?${
      user.userType === 'creator' ? '' : ' AND userId = ?'
    }
  `
...
router.get('/more/channels', requireAuth, (req, res) => {
  ...
  const { user } = req
  return fetchChannels(user, currentChannelId, channelIds)
...
router.get('/more/messages', requireAuth, (req, res) => {
  ...
  `
  return poolQuery(query, [messageId, channelId])
    .then(messages =>
      promiseSeries(
        messages.map(({ id }) => {
          let query = `
        ...
        `
          return () =>
            poolQuery(query, id).then(([message]) => Promise.resolve(message))
        })
      )
    )
...
router.get('/channels', requireAuth, (req, res) => {
  ...
  return fetchChannels(user, currentChannelId)
    .then(channels => res.send(channels))
    .catch(error => {
      console.error(error)
      res.status(500).send({ error })
...
router.get('/channel', requireAuth, (req, res) => {
  ...
  return fetchMessages(channelId)
    .then(messages =>
      poolQuery('UPDATE users SET ? WHERE id = ?', [
        { lastChannelId: channelId },
        user.id
      ]).then(() => Promise.resolve(messages))
    )
    .then(messages => {
      return promiseSeries([
        () =>
        ...
        () =>
          fetchSingleChannel(channelId, user.id).then(channel =>
...
router.post('/channel', requireAuth, (req, res) => {
  ...
  return poolQuery('INSERT INTO msg_channels SET ?', {
    channelName,
    creator: user.id
  })
    .then(result => Promise.resolve(result.insertId))
    .then(channelId => {
      const members = [user.id].concat(
      ...
      updateLastRead({
        users: members.map(member => ({ id: member })),
        channelId,
    ...
    .then(message => {
      ...
      `
      return poolQuery(query, message.channelId).then(members =>
        Promise.resolve({ members, message })
      )
    })
    .then(({ members, message }) => {
...
router.post('/channel/twoPeople', requireAuth, (req, res) => {
  ...
  return poolQuery('INSERT INTO msg_channels SET ?', { twoPeople: true })
    .then(result => {
      updateLastRead({
        users: [{ id: partnerId }, { id: user.id }],
    ...
    .then(channelId =>
      promiseSeries([
        ...
        () =>
          poolQuery('INSERT INTO msg_chats SET ?', {
            channelId,
            userId: user.id,
            content,
            timeStamp
        ...
        Promise.resolve({ channelId, messageId: result.insertId })
      )
    )
    .then(({ channelId, messageId }) =>
      poolQuery('UPDATE users SET ? WHERE id = ?', [
        { lastChannelId: channelId },
        user.id
      ]).then(() => Promise.resolve({ channelId, messageId }))
    )
    .then(({ channelId, messageId }) => {
      let query = `
        SELECT a.userId, b.username FROM
        ...
        a.userId = b.id WHERE a.channelId = ?
      `
      return poolQuery(query, channelId).then(members =>
        res.send({
          id: messageId,
          messageId,
          channelId,
...
router.delete('/channel', requireAuth, (req, res) => {
  ...
  function leaveChannel(callback) {
    let query =
      'DELETE FROM msg_channel_members WHERE channelId = ? AND userId = ?'
  ...
  async.waterfall(
    [
      ...
      (channelName, callback) => {
        ...
        let taskArray = selectedUsers.reduce((taskArray, user) => {
...
router.get('/numUnreads', requireAuth, (req, res) => {
  ...
  return poolQuery(channelQuery, user.id)
    .then(rows => {
      if (rows.length > 0) {
        return promiseSeries(
          rows.map(({ channelId }) => () =>
            poolQuery(lastReadsQuery, [user.id, channelId])
          )
        )
      } else {
    ...
    .then(results => {
      const totalNumUnreads = results.reduce(
        (sum, [{ numUnreads = 0 }]) => sum + Number(numUnreads),
        0
      )
...
router.get('/search/chat', requireAuth, (req, res) => {
  ...
  let query = `
    ...
    LIMIT 10
  `
  return poolQuery(query, [`%${text}%`, `%${text}%`, `%${text}%`, `%${text}%`])
    .then(primaryRes => {
      ...
      `
      if (remainder > 0) {
        return poolQuery(query, [`%${text}%`, `%${text}%`]).then(secondaryRes =>
          Promise.resolve({ primaryRes, secondaryRes })
        )
      }
    ...
    .then(({ primaryRes, secondaryRes }) => {
      let finalRes = primaryRes
        .map(res => Object.assign({}, res, { primary: true }))
        .concat(
          secondaryRes
            .filter(row => {
              let remains = true
            ...
            .map(res => Object.assign({}, res, { primary: false }))
        )
      res.send(finalRes)
    })
    .catch(error => {
...
router.get('/search/users', (req, res) => {
  ...
  if (stringIsEmpty(text) || text.length < 2) return res.send([])
  const query =
    'SELECT * FROM users WHERE (username LIKE ?) OR (realName LIKE ?) LIMIT 5'
  return poolQuery(query, [`%${text}%`, `%${text}%`])
    .then(results => res.send(results))
    .catch(error => {
      console.error(error)
      res.status(500).send({ error })
