...
router.get('/', (req, res) => {
  const linkId =
    typeof req.query.linkId !== 'undefined' ? req.query.linkId : null
  ...
  return poolQuery(query)
    .then(results => {
      ...
      return Promise.all(tasks).then(likersArray => {
...
router.delete('/page', requireAuth, (req, res) => {
  ...
  return poolQuery('DELETE FROM content_urls WHERE id = ? AND uploader = ?', [
    linkId,
    user.id
  ])
...
router.get('/page', (req, res) => {
  ...
  let query = `
    ...
    WHERE a.id = ?
  `
  poolQuery(query, linkId)
    .then(([result]) => {
      let query = `
        SELECT a.userId, b.username FROM content_likes a JOIN users b ON a.userId = b.id
        WHERE a.rootType = 'url' AND a.rootId = ?
      `
      return poolQuery(query, result.id).then(likers =>
        Promise.resolve({ result, likers })
      )
    })
    .then(({ result, likers }) => {
...
router.put('/page', requireAuth, (req, res) => {
  const {
    user,
    body: { editedTitle, editedDescription, editedUrl, linkId }
  } = req
  ...
  return poolQuery('UPDATE content_urls SET ? WHERE id = ? AND uploader = ?', [
    post,
    linkId,
    user.id
  ])
