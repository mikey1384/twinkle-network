...
router.delete('/', requireAuth, (req, res) => {
  ...
  const query = `DELETE FROM ${
    type === 'video' ? 'vq_videos' : `content_${type}s`
  } WHERE id = ?`
...
router.put('/', requireAuth, (req, res) => {
  ...
  switch (type) {
    ...
    case 'video':
      ...
      break
    default:
      return res.status(500).send({ error: 'Type not specified' })
  }
  const query = `UPDATE ${
    type === 'video' ? 'vq_videos' : `content_${type}s`
  } SET ? WHERE id = ?`
  return poolQuery(query, [post, contentId])
    .then(() => res.send(post))
    .catch(error => {
      console.error(error)
      res.status(500).send({ error })
...
router.put('/embed', (req, res) => {
  ...
  return getThumbImageFromEmbedApi({ url })
    .then(response => {
      const {
        image = { url: '' },
        title = '',
...
router.put('/videoThumb', (req, res) => {
  ...
  request({
    ...
    .then(response => {
      ...
      poolQuery('UPDATE vq_videos SET hasHqThumb = ? WHERE id = ?', [
        !!payload,
        videoId
      ])
      res.send({ payload })
