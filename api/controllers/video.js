...
router.get('/', (req, res) => {
  ...
  const videoQuery = `
    ...
    WHERE a.id = ?
  `
  return poolQuery(idQuery, videoId)
    .then(videoIds =>
      promiseSeries(
        videoIds.map(({ id }) => {
...
router.post('/edit/title', requireAuth, (req, res) => {
  ...
  pool.query(
    'UPDATE vq_videos SET ? WHERE id = ? AND uploader = ?',
...
router.post('/edit/page', requireAuth, (req, res) => {
  ...
  const query = `UPDATE vq_videos SET ? WHERE id = ?${
...
router.get('/more/playlistVideos', (req, res) => {
  ...
  `
  return poolQuery(query, [playlistId, videoId])
    .then(playlistVideos => {
...
router.get('/page', (req, res) => {
  ...
  return poolQuery(query, [videoId, videoId])
    .then(rows => {
      ...
      return promiseSeries([
        () =>
          poolQuery(questionQuery, videoId).then(rows => {
        ...
        () => poolQuery(likeQuery, videoId).then(rows => Promise.resolve(rows))
      ])
    })
    .then(results =>
      res.send(
...
router.get('/rightMenu', (req, res) => {
  ...
  return promiseSeries([
    () =>
      poolQuery(playlistQuery(11), videoId).then(rows => {
        return promiseSeries(
          rows.map(({ playlistId, id }) => {
    ...
    () =>
      poolQuery(playlistQuery(6), videoId).then(rows => {
        return promiseSeries(
          rows.map(({ playlistId, id }) => {
            return () =>
              poolQuery(relatedVideosQuery(11), [playlistId, videoId])
          })
        ).then(results => {
          let videos = []
          for (let i = 0; i < results.length; i++) {
    ...
    () =>
      poolQuery(otherVideosQuery, videoId).then(rows => Promise.resolve(rows))
  ])
    .then(results => {
...
router.get('/discussions/comments', (req, res) => {
  ...
  pool.query(query, discussionId, (err, rows) => {
    ...
    returnComments(rows, 'video')
      .then(commentsArray => res.send(commentsArray))
      .catch(err => {
        console.error(err)
        return res.status(500).send({ error: err })
      })
...
router.put('/duration', requireAuth, async(req, res) => {
  ...
  if (isStarred && !xpEarned) {
    const [{ currentlyWatching }] = await poolQuery(
      `SELECT currentlyWatching FROM users WHERE id = ?`,
      user.id
    )
...
router.post('/replies/edit', requireAuth, (req, res) => {
  ...
  pool.query(
    'UPDATE content_comments SET ? WHERE id = ? AND userId = ?',
    [{ content }, replyId, user.id],
    err => {
      if (err) {
        console.error(err)
        return res.status(500).send({ error: err })
  ...
  async.waterfall(
    [
      callback => {
        const userId = user.id
        pool.query(
          'DELETE FROM vq_questions WHERE videoId = ? AND creator = ?',
          [videoId, userId],
          err => {
            callback(err, userId)
          }
...
router.put('/currentlyWatching', requireAuth, async(req, res) => {
  const { user, body: { videoId } } = req
  try {
