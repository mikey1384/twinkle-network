...
module.exports = {
  ...
  fetchComments(req, res) {
    ...
    `
    poolQuery(query, rootId)
      .then(rows => returnComments(rows, rootType))
  ...
  fetchReplies(req, res) {
    ...
    const where =
      !!lastReplyId && lastReplyId !== '0'
        ? 'AND a.id < ' + lastReplyId + ' '
        : ''
    ...
    `
    return poolQuery(query, [commentId, lastReplyId])
      .then(rows => {
        if (rows.length > 10) {
          rows.pop()
        ...
        return returnReplies(rows).then(replies =>
  ...
  likeComments(req, res) {
    ...
    `
    return poolQuery(query1, [commentId, user.id])
      .then(rows => {
        if (rows.length > 0) {
      ...
      .then(() => poolQuery(query4, commentId))
      .then(rows =>
        res.send({
          likes: rows
        })
  ...
  postComments(req, res) {
    ...
    return poolQuery('INSERT INTO content_comments SET ?', post)
      .then(result =>
        res.send(
          Object.assign({}, post, {
            id: result.insertId,
...
function returnComments(commentRows, rootType) {
  ...
  for (let index = 0; index < commentRows.length; index++) {
    ...
    taskArray.push(
      fetchCommentElements({ commentRow, commentsArray, index, rootType })
    )
  }
  return Promise.all(taskArray).then(() => Promise.resolve(commentsArray))
}

const fetchCommentElements = ({
  commentRow,
  commentsArray,
  index,
  rootType
  ...
  return Promise.all([
    ...
    poolQuery(query2, commentId).then(rows => returnReplies(rows))
  ])
    .then(results => {
...
const fetchReplyElements = ({ replyRow, repliesArray, index }) => {
  ...
  let query = `
    ...
    a.rootType = 'comment' AND a.rootId = ?
  `
  return poolQuery(query, replyId).then(rows =>
    Promise.resolve(
      Object.assign({}, replyRow, {
        likes: rows.map(like => ({
          userId: like.userId,
